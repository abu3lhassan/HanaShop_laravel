...
<div class="premium-card p-3 p-lg-4">
    <div class="table-responsive"><table class="table align-middle text-center mb-0"><thead><tr><th>#</th><th>Product</th><th>Price</th><th>Qty</th><th>Color</th><th>Image</th></tr></thead><tbody>@forelse($producdetails as $item)<tr><td>{{ $item->id }}</td><td class="fw-bold">{{ $item->name }}</td><td>SAR {{ number_format($item->price, 2) }}</td><td>{{ $item->qty }}</td><td>{{ $item->color }}</td><td class="text-truncate" style="max-width:180px">{{ $item->image }}</td></tr>@empty<tr><td colspan="6">No product details yet.</td></tr>@endforelse</tbody></table></div>
</div>
@endsection
